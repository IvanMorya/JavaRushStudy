    ...
    public static void main(String[] args) {
        ...
        while (openedTagIndex != -1 || closedTagIndex != -1) {
            if (openedTagIndex != -1 && openedTagIndex < closedTagIndex) {
                ...
                openedTagIndex = fileContent.indexOf(openedTag, ++openedTagIndex);
            } else if (closedTagIndex < openedTagIndex || openedTagIndex == -1) {
                closedTagsIndexes.add(closedTagIndex + tag.length() + 3);
                closedTagIndex = fileContent.indexOf(closedTag, ++closedTagIndex);
            }
        }
        ArrayList<String> listResultTags = new ArrayList<>();
        for (int i = openedTagsIndexes.size() - 1; i >= 0; i--) {
            listResultTags.add(fileContent.substring(openedTagsIndexes.get(i), getCloseTagIndex(closedTagsIndexes, openedTagsIndexes.get(i))));
        }

        for (int i = listResultTags.size() - 1; i >= 0; i--) {
            System.out.println(listResultTags.get(i));
        }
    }

    private static int getCloseTagIndex(ArrayList<Integer> closedTagsIndexes, Integer integer) {
